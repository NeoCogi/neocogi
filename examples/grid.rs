...
fn main() {
    ...
    let mut current_camera_mode = CameraMode::Orbit;
    let camera = Camera::new(Vec3f::zero(), 20.0, Quatf::of_axis_angle(&Vec3f::new(-1.0, 0.0, 0.0), std::f32::consts::PI / 4.0), std::f32::consts::PI / 4.0, 1.0, 0.1, 100.0);
    let mut view3d = View3D::new(camera, Dimensioni::new(1024, 900), Box3f::new(&Vec3f::new(-20.0, -20.0, -20.0), &Vec3f::new(20.0, 20.0, 20.0)));
    ...
    while !window.should_close() {
        ...
        let egui_output = egui_ctx.run(egui_input_state.egui_input.take(), |egui_ctx| {
            egui::SidePanel::left("Test").show(&egui_ctx, |ui| {
                ...
                ui.selectable_value(&mut current_camera_mode, CameraMode::Pan, "Pan");
                ui.selectable_value(&mut current_camera_mode, CameraMode::Orbit, "Orbit");
                ui.selectable_value(&mut current_camera_mode, CameraMode::Zoom, "Zoom");
            });
        ...
        um_renderer.draw_node(&view3d.pvm(), &grid);
        ...
        let pointer_button  = if window.get_mouse_button(glfw::MouseButtonLeft) == glfw::Action::Press { pointer::ButtonState::Pressed(1.0) } else { pointer::ButtonState::Released };
